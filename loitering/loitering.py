...
def main():
    ...
    while ret:
        ...
        clss = results[0].boxes.cls.cpu().tolist() if results[0].boxes.cls is not None else None
        names = results[0].names if results[0].names is not None else None
        

       
        if boxes is not None and track_ids is not None and clss is not None:
