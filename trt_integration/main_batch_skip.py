...
class YoloTRT():
    ...
    def Inference(self, frames):
        ...
        input_images, raw_images, origin_hs, origin_ws = self.PreProcessImg(img)
        np.copyto(host_inputs[0], input_images.ravel())
        stream = cuda.Stream()
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.PostProcess(output[i * self.det_output_length: (i + 1) * self.det_output_length], origin_hs[i], origin_ws[i])            
