...
class YoloTRT():
    ...
    #         raw_imgs.append(image_raw)
    #         orig_hs.append(h)
    #         orig_ws.append(w)
           
    #     return np.vstack(preprocessed_imgs), raw_imgs, orig_hs, orig_ws 
    ...
    def Inference(self, frames):
        ...
        batch_origin_h = []
        batch_origin_w = []
        batch_input_image = np.empty(shape=[self.batch_size, 3, self.input_h, self.input_w])  # Create an empty NumPy array with shape [batch_size, channels, height, width]
        
        # Iterate over imgs to preprocess each image in a batch
        for i, image_raw in enumerate(imgs):
            # Preprocess each image
            input_image, image_raw, origin_h, origin_w = self.PreProcessImg(image_raw)

            # Append the preprocessed image_raw, origin_h, and origin_w to their respective batches
            batch_image_raw.append(image_raw)
            batch_origin_h.append(origin_h)
            batch_origin_w.append(origin_w)

            # Copy the preprocessed input_image to the appropriate index in batch_input_image
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.PostProcess(output[i * self.det_output_length: (i + 1) * self.det_output_length], batch_origin_h[i], batch_origin_w[i])            
            ...
            for j in range(len(result_boxes)):
                ...
                det["box"] = box            
                det_res.append(det)
                self.PlotBbox(box, imgs[i], color = colors(int(result_classid[j]), True), label="{}:{:.2f}".format(self.categories[int(result_classid[j])], result_scores[j]),)
            all_results.append(det_res)
            
        return all_results, t2-t1
...
def process_video(source):
    ...
    while cv.waitKey(1) != 27: #ESC key
        ...
        # Perform batch detection & tracking on frame
        if len(frame_batch) == batch_size:

            # YOLO Inference with TensorRT
            detections_batch, t = model.Inference(frame_batch)
            logger.info("Detections: {}".format(detections_batch))
            ...
            for frame, detections in zip(frame_batch, detections_batch):
                for i in range(20):
                    ...
                    print("frames cnt", frames_cnt[i])
                    module_result.append([frames_cnt[i], 
                            crowd_density, 
                            loitering, 
            ...
            frame_batch = []
            frames_cnt = [] 
            print("module result: ", module_result)               
            if (not persist) and (module_result != []):
                print("inside RBP")
